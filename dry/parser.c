/****************************************************/
/*  Parser for mtm3 - written by Asaf Rubinfeld     */
/****************************************************/
                                 ...
                                 int *roomNumOfPpl, int *roomOpenHour, int *roomCloseHour, int *roomDifficulty)
{
    ...
    *roomNumOfPpl = atoi( strtok(NULL, delimiter) );
    ...
    if(parameter == NULL || strlen(parameter) != 5 || strchr(parameter, '-') == NULL ){
        return MTM_INVALID_PARAMETER;
    }
    *roomDifficulty =  atoi( strtok(NULL, delimiter) );
    *roomOpenHour = atoi( strtok(parameter, " \n\t-") );
                                      ...
                                      int *requestedTime, int *requestedDay, int *roomNumOfPpl )
{
    ...
    *escaperEmail = malloc(strlen(parameter) + 1);
    ...
    int check = atoi (strtok(NULL, delimiter));
    *faculty = stringToFaculty( check );
    *roomId = atoi( strtok(NULL, delimiter) );
    parameter =  strtok(NULL, delimiter);
    if(sscanf(parameter,"%d-%d%c", requestedDay, requestedTime, &junk) != 2){
        return MTM_INVALID_PARAMETER;
    }
    /*
    ...
    */
    //*requestedDay = atoi( strtok(NULL, " \n-") );
    //*requestedTime = atoi( strtok(parameter, delimiter) );
    *roomNumOfPpl = atoi( strtok(NULL, delimiter) );
...
/* parameters to parse : <escaper email>  <numOfPpl>          */
/**************************************************************/
static MtmErrorCode parseEscaperRecommend(char **escperEmail, int *roomNumOfPpl)
{
    char *parameter;
